...
public class HandlerManager {
    ...
    private static class HandlerRegistry {
        ...
        private final HashMap<VaadinEvent.Type<?>, ArrayList<?>> map = new HashMap<VaadinEvent.Type<?>, ArrayList<?>>();

        private <H extends java.lang.reflect.InvocationHandler> void addHandler(VaadinEvent.Type<H> type, H handler) {
        ...
        private <H extends java.lang.reflect.InvocationHandler> void fireEvent(VaadinEvent<H> event,
                boolean isReverseOrder) {
            VaadinEvent.Type<H> type = event.getAssociatedType();
        ...
        private <H> ArrayList<H> get(VaadinEvent.Type<H> type) {
            // This cast is safe because we control the puts.
            return (ArrayList<H>) map.get(type);
        }

        private <H extends java.lang.reflect.InvocationHandler> H getHandler(VaadinEvent.Type<H> eventKey,
                int index) {
            ArrayList<H> l = get(eventKey);
            return l.get(index);
        }

        private int getHandlerCount(VaadinEvent.Type<?> eventKey) {
            ArrayList<?> l = map.get(eventKey);
            return l == null ? 0 : l.size();
        }

        private boolean isEventHandled(VaadinEvent.Type<?> eventKey) {
            return map.containsKey(eventKey);
        }

        private <H> void removeHandler(VaadinEvent.Type<H> eventKey, H handler) {
            ArrayList<H> l = get(eventKey);
     ...
     *         handler later
     */
    public <H extends java.lang.reflect.InvocationHandler> HandlerRegistration addHandler(
            VaadinEvent.Type<H> type, final H handler) {
     ...
     */
    public <H extends java.lang.reflect.InvocationHandler> H getHandler(VaadinEvent.Type<H> type, int index) {
     ...
     */
    public int getHandlerCount(VaadinEvent.Type<?> type) {
     ...
     */
    public boolean isEventHandled(VaadinEvent.Type<?> e) {
     ...
     */
    @Deprecated
    public <H extends java.lang.reflect.InvocationHandler> void removeHandler(VaadinEvent.Type<H> type,
            final H handler) {
        if (firingDepth > 0) {
     ...
     */
    Map<VaadinEvent.Type<?>, ArrayList<?>> createHandlerInfo() {
        return registry.map;
    }
    ...
    private <H extends java.lang.reflect.InvocationHandler> void doAdd(VaadinEvent.Type<H> type,
            final H handler) {
        registry.addHandler(type, handler);
    }

    private <H extends java.lang.reflect.InvocationHandler> void doRemove(VaadinEvent.Type<H> type,
            final H handler) {
        registry.removeHandler(type, handler);
    }

    private <H extends java.lang.reflect.InvocationHandler> void enqueueAdd(final VaadinEvent.Type<H> type,
            final H handler) {
    ...
    private <H extends java.lang.reflect.InvocationHandler> void enqueueRemove(
            final VaadinEvent.Type<H> type, final H handler) {
        defer(new AddOrRemoveCommand() {
